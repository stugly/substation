    ...
    targetSIDs.forEach((sid) => {
        ...
        const getShiftLogs = (shiftName, dateStr, minHM, maxHM) => {
            return fullCheckins.filter(cp => {
                ...
                return cp.sid === sid && (cp.job || "").toString().trim() === shiftName && logDate === dateStr && logHM >= minHM && logHM <= maxHM && cTime <= now;
            });
        };

        // --- 2. Logic แยกตามประเภทงาน ---
        ...
        if (isDayTimeType) {
            ...
            // 🚩 [เงื่อนไข] กะ 2 และ กะ 3
            if (currentTimeHM < 800) {
                // ก่อน 8.00: หา กะ 2 วันนี้ ถ้าไม่มี ให้โชว์ กะ 3 เมื่อวาน
                const logsG2 = getShiftLogs("เข้าปฏิบัติงานกะ 2", todayStr, 700, 800);
                if (logsG2.length > 0) displayLog = logsG2[0];
                else displayLog = getShiftLogs("เข้าปฏิบัติงานกะ 3", yesterdayStr, 1500, 2300).sort((a,b) => new Date(b.time) - new Date(a.time))[0];
                statusText = "เข้าปฏิบัติงานกะ 2";
                ...
                // 15.00-16.00: หา กะ 3 วันนี้ ถ้าไม่มี ให้โชว์ กะ 2 วันนี้
                const logsG3 = getShiftLogs("เข้าปฏิบัติงานกะ 3", todayStr, 1500, 1600);
                if (logsG3.length > 0) displayLog = logsG3[0];
                else displayLog = getShiftLogs("เข้าปฏิบัติงานกะ 2", todayStr, 700, 1500).sort((a,b) => new Date(b.time) - new Date(a.time))[0];
                statusText = "เข้าปฏิบัติงานกะ 3";
            } else {
            ...
            if (displayLog) {
                const chkHM = new Date(displayLog.time).getHours() * 100 + new Date(displayLog.time).getMinutes();
                if (displayLog.job === "เข้าปฏิบัติงานกะ 2") {
                ...
                } else if (displayLog.job === "เข้าปฏิบัติงานกะ 3") {
